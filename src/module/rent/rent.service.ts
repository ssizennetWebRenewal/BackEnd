...
export class RentService {
  ...
  async applyRent(
        ...
        return rent.equipmentList.some((equipment: any) =>
          apply.equipmentList.some(
            (newEquipment: any) =>
              ...
              equipment.items.some((item: string) =>
                newEquipment.items.includes(item),
              ),
          ),
        );
      }
    ...
    await newRent.save();
    this.logger.log(
      `대여 신청: ${newRent.title}, ${newRent.applicantName} (${newRent.id})`,
    );
    return newRent;
  }
  ...
  async monthRented(year: number, month: number): Promise<any[]> {
    ...
    const rents = await this.RentsModel.query('constTrue')
      ...
      .where('combinedDate')
      .between(
        `${startOfMonth}#0000-00-00T00:00:00.000Z`,
        `${endOfMonth}#9999-99-99T99:99:99.999Z`,
      )
      .using('DateIndex')
      .exec();
  ...
  async getRentById(id: string): Promise<any> {
    const rent = await this.RentsModel.query('id')
      .eq(id)
      .exec()
      .then((res) => res[0]);
    if (!rent) {
  ...
  async updateRent(
    ...
    const startTime = updateRentDto.startDate
      ? new Date(updateRentDto.startDate).toISOString()
      : rent.startDate;
    const endTime = updateRentDto.endDate
      ? new Date(updateRentDto.endDate).toISOString()
      : rent.endDate;
