...
export class VideoService {
  ...
  async regVideo(
    ...
    await video.save();
    this.logger.log(
      `비디오 등록: ${createVideoDto.title}, ${user.id} (${video.id})`,
    );
    return {
      id: video.id,
  ...
  async updateVideo(
    ...
    if (
      !user.authority.includes('영상관리자') ||
      (user.authority.includes('사용자') && video.writer !== user.id)
    ) {
  ...
  async approveVideo(
    id: string,
    approveVideoDto: ApproveVideoDto,
    user: any,
  ): Promise<VideoResponseDto> {
    const video = await this.videoModel
      .query('id')
