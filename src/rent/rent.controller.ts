...
export class RentController {
    ...
    async applyRent(@Body() applyRentDto: ApplyRentDto, @Req() req: CustomRequest, @Res() res: Response) {
        const newRent = await this.rentService.applyRent(applyRentDto, req.user.id, req.user.name);
    ...
    async getRent(@Query('page') page: string, @Query('count') count: string, @Res() res: Response) {
    ...
    @Roles('장비관리자')
    @Post('approveRent')
    @ApiOperation({ summary: '대여 승인', description: '신청된 대여를 승인합니다.' })
    @ApiBody({ type: ApproveRentDto })
    @ApiResponse({ status: 200, description: '대여 승인 완료' })
    @ApiResponse({ status: 403, description: '권한 부족' })
    async approveRent(@Body() approveRentDto: ApproveRentDto, @Res() res: Response) {
